...
<Note>
  Accessing `ctx.runtime.serverInfo` requires `deepagents>=1.9.0`. On older versions, read the assistant ID from `getConfig().metadata.assistantId` instead.
</Note>
...
Deep Agents already use [checkpointers](/oss/javascript/langgraph/persistence#checkpoints) which is the mechanism that supports episodic memory: every conversation is persisted as a checkpointed thread.
...
new StoreBackend({
  namespace: (ctx) => [
    ctx.runtime.serverInfo.assistantId,  // [!code highlight]
